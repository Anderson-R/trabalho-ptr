    	...
    	case ESP_GATTS_WRITE_EVT: {
            ...
            if(p_data->write.is_prep == false){
                ...
                if(res == SPP_IDX_SPP_COMMAND_VAL){
                    ...
                    if(p_data->write.len == 9){
                        int dataId = p_data->write.value[0];
                        float hum = (p_data->write.value[1]<<24 | p_data->write.value[2]<<16 | p_data->write.value[3]<<8 | p_data->write.value[4])/1000000.0;
                        float temp = (p_data->write.value[5]<<24 | p_data->write.value[6]<<16 | p_data->write.value[7]<<8 | p_data->write.value[8])/1000000.0;
                        ESP_LOGE("test", "id: %d \t hum: %f \t temp: %f", dataId, hum, temp);
                    }
#endif
                }else{
                    //TODO:
